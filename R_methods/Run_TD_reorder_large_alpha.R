...
source("/Users/tongxu/Downloads/projects/MICODAG-CD/R_methods/EqVarDAG_TD.R")
...
for (dataset in datasets) {
  ...
  for (kk in c(1:10)) {
    ...
    random_order <- sample(p)  # random permutation of 1:P
    true_dag <- as(graph_ori, "matrix")
    new_true_dag <- true_dag[random_order, random_order]
    graph_ori <- as(new_true_dag, "graphNEL")
    est.moral.graph <- est.moral.graph[random_order, random_order]
    new_X <- X[, random_order]  # assuming columns are in the correct order
    
    
    # run
    start_time <- Sys.time()
    result <- EqVarDAG_TD(new_X)
    end_time <- Sys.time()
...
write.csv(results, "./experiment results/comparison with benchmarks/TD_RealGraph_estimated_superstructure_reorder_large_diff.csv",row.names=FALSE)
