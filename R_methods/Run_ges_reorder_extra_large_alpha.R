...
for (dataset in datasets) {
  for (kk in c(1:10)) {
    ...
    write.table(ordering, file = paste0(paste(dataset.folder,dataset,sep="/"), "/ges_topo_order_iter", kk,".txt"), row.names = FALSE, col.names = FALSE)
...
print(results)
# write.csv(results, "./experiment results/comparison with benchmarks/ges_results_est_default_reorder_extra_large_diff.csv",row.names=FALSE)
