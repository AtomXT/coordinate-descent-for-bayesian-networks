...
for (dataset in datasets) {
  ...
  for (kk in c(1:10)) {
    ...
    # run
    start_time <- Sys.time()
    result <- EqVarDAG_TD(X, ordering=new_ordering, mtd='rls',threshold=sqrt(6*log(nodes)/samples))
    end_time <- Sys.time()
...
write.csv(results, "./experiment results/cd_vs_regression/TD_regression_slightly_incorrect_ordering_small1_diff.csv",row.names=FALSE)
