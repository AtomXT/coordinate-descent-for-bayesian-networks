...
from src.cd_spacer import CD_order
import time
import pandas as pd
import numpy as np
...
# n_samples = [5000]
n_samples = [50, 100, 200, 300, 400, 500, 800, 1000, 1600, 3200]
for seed_i, seeds in enumerate([33, 100, 200]):
    np.random.seed(seeds)
    random_ordering = np.arange(10)
    ...
    for graph_i in range(1, 2):
        for n_sample in n_samples:
            for iter in range(1, 11):
                ...
                start = time.time()
                est_CD, obj = CD_order(data, true_moral, ordering=random_ordering, MAX_cycles=400, lam=np.sqrt(5*np.log(P) / N), cholesky=True)
                end = time.time()
                time_i = end-start
    ...
    result_cd_df.to_csv(f'./experiment results/synthetic_results_CD_order_small_diff_random_ordering{seed_i}.csv', index=False,header=True)
    print(result_cd_df)
