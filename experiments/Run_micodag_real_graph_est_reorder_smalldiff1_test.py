...
if __name__ == '__main__':
    ...
    for dataset in ['1dsep', '2asia']:
        for kk in range(1, 11):
            ...
            start_i = time.time()
            RGAP, est, _, _, run_time = mic.optimize(data, moral_, np.sqrt(4 * np.log(P) / n), gurobi_params=gurobi_parameters)
            end_i = time.time()
            ...
            print(df)
            # result_file = os.path.join(current_dir, "../experiment results/comparison with benchmarks/micodag_est_12log(m)n_small_diff1_job1.csv")
