...
for dataset in datasets:
    ...
    for iter in range(1, 11):
        ...
        start = time.time()
        est, _ = CD_order(data, estimated_moral, ordering=np.arange(P), MAX_cycles=400, lam=np.sqrt(12*np.log(P) / N), cholesky=True)
        end = time.time()
...
# results_df.to_csv('./experiment results/cd_vs_regression/CD_correct_order_large_diff.csv', index=False, header=True)
