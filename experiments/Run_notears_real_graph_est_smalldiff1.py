...
for dataset in datasets:
    ...
    for iter in range(1, 11):
        ...
        start = time.time()
        est = model.fit(data.values, lambda1=0.15, exclude_edges=exclude_edges, max_iter=3000, warm_iter=1000)
        end = time.time()
        ...
        print(results_df)
        print(np.mean(results_df.d_cpdag))
        results_df.to_csv(f'./experiment results/comparison with benchmarks/notears_est_small_diff1.csv', index=False, header=True)
