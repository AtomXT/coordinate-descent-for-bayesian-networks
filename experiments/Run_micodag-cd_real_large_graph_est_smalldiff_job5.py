...
datasets = ['14munin']
nsamples = {'13pathfinder':5450, '14munin':9300, '15andes':11150, '16diabetes':20650}
results = []
for dataset in datasets:
    d_cpdags = []
    times = []
    for iter in range(1, 11):
        data, true_dag, estimated_moral, true_moral = read_data(dataset, nsamples[dataset], iter)
        N, P = data.shape
        ...
        start = time.time()
        est, _ = CD_order(data, estimated_moral, MAX_cycles=400, tol=1e-2, lam=np.sqrt(5*np.log(P) / N), cholesky=True)
        end = time.time()
        ...
        print(results_df)
        results_df.to_csv('./experiment results/comparison on large graphs/RealGraph_large_est_small_diff_part5.csv', index=False, header=True)
