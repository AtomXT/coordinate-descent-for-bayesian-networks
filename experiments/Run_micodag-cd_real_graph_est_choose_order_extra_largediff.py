...
def read_data(network, n=500, iter=1):
    ...
    ges_ordering = pd.read_table(folder_path + f"{network}/ges_topo_order_iter{iter}.txt", header=None).values.squeeze()
    td_ordering = pd.read_table(folder_path + f"{network}/td_topo_order_iter{iter}.txt", header=None).values.squeeze()
...
for dataset in datasets:
    ...
    for iter in range(1, 11):
        data, true_dag, moral_lasso, true_moral, ges_o, td_o = read_data(dataset, 500, iter)
        N, P = data.shape


        est_ges, _ = CD_order(data, moral_lasso, ordering=ges_o, MAX_cycles=400, lam=np.sqrt(5*np.log(P) / N), cholesky=True)
        est_td, _ = CD_order(data, moral_lasso, ordering=td_o, MAX_cycles=400, lam=np.sqrt(5*np.log(P) / N), cholesky=True)

        sigma_hat = np.cov(data.T)
        theta_hat_ges, theta_hat_td = est_ges @ est_ges.T, est_td @ est_td.T
        bic_ges = -N*(np.log(np.linalg.det(theta_hat_ges)) - np.trace(sigma_hat @ theta_hat_ges)) 
        + np.sum(np.triu(np.abs(theta_hat_ges) > 1e-8)) * np.log(N)
        bic_td = -N*(np.log(np.linalg.det(theta_hat_td)) - np.trace(sigma_hat @ theta_hat_td)) 
        + np.sum(np.triu(np.abs(theta_hat_td) > 1e-8)) * np.log(N)
        winner = "ges" if bic_ges < bic_td else "td"
        print(f"For {dataset} iter {iter}, {winner} wins.")
