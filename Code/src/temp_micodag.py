...
def optimize(data, moral, lam, timelimit=50, verbose=1):
    ...
    for k in range(p):
        for j in range(p):
            for i in range(p):
                trace += Gamma[k, i] * Gamma[j, i] * Q[j, k]

    perspective_bigM = gp.LinExpr()
    perspective = gp.LinExpr()
    for j in range(p):
        for i in range(p):
            perspective_bigM += Gamma[j, i]*Gamma[j, i]*D[j, j]

    for j in range(p):
        perspective += D[j, j] * S_var[j, j]
        for k in range(p):
            if (j, k) in list_edges:
                perspective += D[k, k]*S_var[k, j]
    ...
    B = np.eye(p) - Gamma_opt@np.linalg.inv(D_half)
    B[abs(B) <= 1e-6] = 0
    run_time = end - start
    RGAP = m.MIPGAP

    return [RGAP, Gamma_opt, run_time, m.ObjVal]